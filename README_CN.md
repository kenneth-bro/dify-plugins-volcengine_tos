...
一个功能强大的 Dify 插件，提供与火山引擎对象存储服务（TOS）的无缝集成。支持将文件直接上传至 TOS，并基于 URL 高效获取文件内容，支持批量操作和公共文件下载，提供丰富的配置选项。
...
### 版本历史
- v0.0.2（2025-12-27）：新增批量下载文件、公共文件下载功能, 修复Bug
...
### 批量下载文件
- 支持使用分号分隔的多个 URL 一次性下载多个文件
- 高效批量处理：在单个操作中处理多个文件
- 详细状态报告：跟踪每个文件的成功/失败状态

### 公共文件下载
- 无需认证：从任何平台下载公共文件，无需 API 密钥
- 通用兼容性：适用于任何可公开访问的 URL
- 灵活的 SSL 选项：根据需要配置 SSL 验证
...
- Python 3.12
- 火山引擎账户（有效的 AccessKey 凭据，用于 TOS 操作）
- Dify 平台访问权限
...
- 参数：
  - urls：多个文件 URL，用分号（;）分隔（必填）
    - 示例：`url1;url2;url3`

### 5. 下载公共文件（get_public_file_by_url）
- 参数：
  - url：要下载的公共文件 URL（必填）
  - enable_verify_ssl：启用 SSL 证书验证（默认：true）
...
- 确保 TOS 存储桶已正确配置权限
- 插件需要具备 TOS 访问权限的有效凭据（用于 TOS 操作）
- 公共文件下载不需要任何凭据
- 超大文件建议使用分片上传（当前未实现）
